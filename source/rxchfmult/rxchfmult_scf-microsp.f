!======================================================================
      subroutine RXCHFmult_scf_microsp(nelec,NAE,NBE,NPRA,NPRB,NUCST,
...
!--------OUTPUT-FORMATTING---------------------------------------------(
 ...
 3001 FORMAT(/1X,'STARTING SPECIAL MICROITERATIONS FOR ITERATION',1X,I3)

 3000 FORMAT(1X,'CONVERGED REGULAR MICROITERATION',1X,I3,1X,'IN',
     x       1X,I3,1X,'MICROITERATIONS',/)
                                           
!--------OUTPUT-FORMATTING---------------------------------------------)

      if(.not.LOCBSE) then
       write(*,*) "LOCBSE IS REQUIRED FOR THIS TYPE OF SCF"
       return
      end if
...
! Reform Fock matrices responding to latest reg electron step
...
!        --> PRINT SUMMARY OF THIS ITERATION
         if((LSOSCFA).and.(LSOSCFB)) then
            WRITE(*,9151) isp,E_total,Delta_E_tot,
     x                    DIFFAE,DIFFBE,DIFFP,ORBGRDA,ORBGRDB
         else if ((LSOSCFA).and.(.not.(LSOSCFB))) then
            WRITE(*,9150) isp,E_total,Delta_E_tot,
     x                    DIFFAE,DIFFBE,DIFFP,ORBGRDA
         else if ((LSOSCFB).and.(.not.(LSOSCFA))) then
            WRITE(*,9150) isp,E_total,Delta_E_tot,
     x                    DIFFAE,DIFFBE,DIFFP,ORBGRDB
         else
            WRITE(*,9100) isp,E_total,Delta_E_tot,
...
! Output the vectors for this iteration for restart if necessary:
         ...
         IF(DIFFBE.LT.TOLE) GOTO 300
         if(ISP.EQ.MAXMICROIT) GOTO 10

       END DO ! special microiterations

  300 CONTINUE
!      IF WE GET HERE - SPECIAL MICROITERATION CONVERGENCE ACHIEVED
       write(*,3000) ielec,isp

         IF(DIFFAE.LT.TOLE) GOTO 200
